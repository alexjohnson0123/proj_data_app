...
import { getProjectTypes, createProjectType, renameProjectType, addAttributeDef, deleteAttributeDef } from '@/api/projectTypes'
          ...
          {projectTypes.map(pt => (
            <div key={pt.id} className="rounded-md border p-5">
              <div className="flex items-center justify-between mb-3">
                <h2 className="font-semibold">{pt.name}</h2>
                {isAdmin && (
                  <Button variant="outline" size="sm" onClick={() => setEditingType(pt)}>
                    Edit
                  </Button>
                )}
              </div>
              {pt.attributeDefs.length === 0 ? (
                ...
                <div className="grid grid-cols-[1fr_auto] gap-x-6 gap-y-2 text-sm max-w-lg">
...
function EditProjectTypeDialog({ projectType, onOpenChange, onSuccess }: EditProjectTypeDialogProps) {
  const [name, setName] = useState(projectType.name)
  const [pendingDeletes, setPendingDeletes] = useState<string[]>([])
  const [pendingAdds, setPendingAdds] = useState<AttributeDefInput[]>([])
  const [newLabel, setNewLabel] = useState('')
  const [newDataType, setNewDataType] = useState<DataType>('string')

  const mutation = useMutation({
    mutationFn: async () => {
      let currentName = projectType.name
      if (name.trim() !== projectType.name) {
        await renameProjectType(projectType.name, name.trim())
        currentName = name.trim()
      }
      for (const label of pendingDeletes) {
        await deleteAttributeDef(currentName, label)
      }
      for (const attr of pendingAdds) {
        await addAttributeDef(currentName, attr)
      }
    },
    onSuccess,
  })

  const existingAttrs = projectType.attributeDefs.filter(a => !pendingDeletes.includes(a.label))

  function stageAdd() {
    if (!newLabel.trim()) return
    setPendingAdds(a => [...a, { label: newLabel.trim(), dataType: newDataType }])
    setNewLabel('')
    setNewDataType('string')
  }

  function unstageAdd(label: string) {
    setPendingAdds(a => a.filter(attr => attr.label !== label))
  }

  function stageDelete(label: string) {
    setPendingDeletes(d => [...d, label])
  }
      ...
      <DialogContent className="max-w-lg">
        ...
        <div className="space-y-5 mt-2">
          ...
          <div className="space-y-2">
            ...
            {(existingAttrs.length > 0 || pendingAdds.length > 0) && (
              <div className="rounded-md border divide-y text-sm mb-2">
                {existingAttrs.map(attr => (
                  <div key={attr.label} className="flex items-center justify-between px-3 py-2">
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <button
                        type="button"
                        onClick={() => stageDelete(attr.label)}
                ...
                {pendingAdds.map(attr => (
                  <div key={attr.label} className="flex items-center justify-between px-3 py-2 bg-muted/30">
                    <span>{attr.label}</span>
                    <div className="flex items-center gap-3">
                      <Badge variant="outline">{attr.dataType}</Badge>
            ...
            <div className="flex gap-2 items-center">
              <Input
                ...
                onKeyDown={e => { if (e.key === 'Enter') { e.preventDefault(); stageAdd() } }}
              />
              ...
              <Button type="button" variant="outline" onClick={stageAdd} disabled={!newLabel.trim()}>
                Add
              </Button>
            </div>
          </div>
...
function CreateProjectTypeDialog({ open, onOpenChange, onSuccess }: CreateProjectTypeDialogProps) {
  ...
  function addAttr() {
    if (!newLabel.trim()) return
    setAttrs(a => [...a, { label: newLabel.trim(), dataType: newDataType }])
    setNewLabel('')
    setNewDataType('string')
  }
