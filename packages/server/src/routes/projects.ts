...
import { attributeValueSchema, changeProjectTypeSchema, valueSchema } from '@proj/shared'
...
router.put('/:id/project-type', validateRequestBody(changeProjectTypeSchema), async (req, res) => {
    const workdayId = req.params.id as string;
    const { projectTypeId, clearAttributes } = req.body;
    const updated = await services.updateProjectType(prisma, workdayId, projectTypeId, clearAttributes);
    res.status(200).json(updated);
});

// Add a new attribute value to a project
router.post('/:id/attributes', validateRequestBody(attributeValueSchema), async (req, res) => {
